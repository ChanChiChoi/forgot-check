...
【可以直接下载右侧Releases】中我构建好的apk进行安装。
...
- **Android 11+ 后台定位提示**：主界面会提示是否缺少“始终允许”位置权限，避免后台监控看似开启但实际无法定位
...
> **Android 11+ / 14 注意**：仅授予前台位置权限时，右上角监控虽然可以开启，但 App 退到后台后 `LocationMonitorService` 可能拿不到位置更新。必须在系统设置里把位置权限改成“始终允许”，后台轮询和提醒才会真正生效。
